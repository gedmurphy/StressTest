...
StressTestWindow::StressTestWindow(void) :
    m_hMainWnd(NULL),
    m_hStatusBar(NULL),
    m_hToolBar(NULL),
    m_CmdShow(0),
...
{
    m_szMainWndClass = L"StressWndClass";
}
...
bool
StressTestWindow::Create (
    _In_ HINSTANCE Instance,
    _In_opt_ LPCWSTR CommandLine,
    _In_ int CmdShow
    )
{
    StressTestWindow MainWindow;
    INITCOMMONCONTROLSEX icex;
    CAtlStringW szAppName;
    int Ret = 1;

    // Store the instances
    g_hParentInstance = Instance;
    g_hThisInstance = Instance;// GetModuleHandleW(L"devmgr.dll");
    ...
    if (szAppName.LoadStringW(g_hThisInstance, IDS_APPNAME))
    {
        ...
        if (MainWindow.Initialize(szAppName, CmdShow))
        {
            // Run the application
            Ret = MainWindow.Run();
...
bool
StressTestWindow::Initialize(_In_z_ LPCTSTR lpCaption,
                           _In_ int nCmdShow)
{
...
void
StressTestWindow::Uninitialize(void)
{
...
bool
StressTestWindow::MainWndMenuHint(_In_ WORD CmdId,
...
void
StressTestWindow::UpdateStatusBar(_In_ bool InMenuLoop)
{
...
bool
StressTestWindow::CreateStatusBar(void)
{
...
bool
StressTestWindow::CreateToolBar(void)
{
...
bool
StressTestWindow::StatusBarLoadString(_In_ HWND hStatusBar,
                                    _In_ INT PartId,
...
LRESULT
StressTestWindow::OnCreate(_In_ HWND hwnd)
{
    LRESULT RetCode;
...
LRESULT
StressTestWindow::OnNotify(_In_ LPARAM lParam)
{
...
LRESULT
StressTestWindow::OnContext(_In_ LPARAM lParam)
{
...
LRESULT
StressTestWindow::OnCommand(_In_ WPARAM wParam,
                          _In_ LPARAM /*lParam*/)
{
    LRESULT RetCode = 0;
    //WORD Msg;
...
void
StressTestWindow::OnActivate(void)
{
    //m_DeviceView->SetFocus();
}

LRESULT
StressTestWindow::OnDestroy(void)
{
...
LRESULT CALLBACK
StressTestWindow::MainWndProc(_In_ HWND hwnd,
                            _In_ UINT msg,
                            _In_ WPARAM wParam,
                            _In_ LPARAM lParam)
{
    StressTestWindow* This;
    LRESULT RetCode = 0;

    // Get the object pointer from window context
    This = (StressTestWindow *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
    if (This == NULL)
    {
    ...
    switch(msg)
    {
        case WM_CREATE:
        {
            ...
            This = (StressTestWindow *)((LPCREATESTRUCT)lParam)->lpCreateParams;
    ...
    )
{
    ...
    return stressTestWindow.Create(Instance, CmdLine, ShowCmd);
}